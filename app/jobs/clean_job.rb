...
class CleanJob < ApplicationJob
  ...
  def delete_expired_records(model, attr: :updated_at, date: Time.current, retention_period: nil, **_opts)
  ...
  def compress_past_records(model, keys, attr: :updated_at, date: Time.current, nocompress_period: nil, **_opts)
    nocompress_period ||= model.nocompress_period
    ...
    model.distinct.pluck(*keys).sum do |data|
      ...
      compress_relation = model.where(condition).where({attr => (...compress_date)})
