...
class RadiusCheckAllJob < ApplicationJob
  ...
  def perform
    # MACアドレス
    mac_address_list = Nic.where.not(mac_address_data: nil).where(auth: true).map(&:mac_address_raw)

    # ユーザー名
    username_list =  User.includes(:assignments).where(assignments: {auth: true}).where(deleted: false)
      .map(&:username)
